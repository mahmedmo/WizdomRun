...
attack NUMERIC(2,1) NOT NULL CHECK (attack >= 1 AND attack <= 5),
...
BEGIN
    ...
    -- If inserting a new answer
    IF TG_OP = 'INSERT' THEN
        IF NOT (answer_count + 1 = 2 OR answer_count + 1 = 4) THEN
            RAISE EXCEPTION 'Each question must have exactly 2 or 4 answers.';
        END IF;
    END IF;
    
    -- If deleting an answer
...
-- Update the trigger to handle both INSERT and DELETE cases
CREATE TRIGGER check_answer_count
BEFORE INSERT OR DELETE ON answers
FOR EACH ROW
